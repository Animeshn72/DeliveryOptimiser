...
1. **Initialization:** The application (`src/main.py`) reads the test cases from `src/test_cases.txt`, where each test case includes the delivery executive's coordinates of current location, list of restaurants and list of consumers. In the list of restaurants, each item of the list contains name of the restaurant, it's location coordinates and preparation time. In list of consumers, each item of the list contains name of the consumer, it's location coordinates. The length of both lists are the same.
